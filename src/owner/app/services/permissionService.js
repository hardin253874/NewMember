(function () {
    ...
    function permissionService ($rootScope, $route, $timeout, config ,ux) {
        ...
        var self = {
            ...
            maxRole: maxRole,
            possibleRoles: {
                //['Manual', 'Weekly', 'Fortnightly', { name: 'Twice Monthly', id: 'TwiceMonthly'} , 'Monthly']
        ...
        function init(sessionVal) {
            ...
            self.isAgent = !!(session.AgentAccess);

            setDebugMode();
            addAdminFeatures();

            // It's better to add to permission service, reason
            $rootScope.isPortalPage = true;
        ...
        function isPortalPage()
        {
            ...
            if (!current) {
                return false;
            } else if (current === '/app/defaultRedirect.html' && !self.isAgent) {
                return true;
            } else {
                return current.startsWith('/app/own/');
            }
        }
        ...
        function setDebugMode() {
            self.isDebugMode =  session.RegisteredEmail.contains('propertyme.com');
            if (self.isDebugMode) { addDebugFns();}
        }
        function checkRole(role) {
            if (!session || !session.AgentAccess || !session.AgentAccess.MemberRoles) return false;
            return session.AgentAccess.MemberRoles.contains(role) || session.AgentAccess.MemberRoles.contains(role.toLowerCase());
        }
        ...
        function addDebugFns() {
                window.addRole = function (r) {session.AgentAccess.MemberRoles.push(r); setEditAllowed();};
                window.setRole = function (r) {session.AgentAccess.MemberRoles = [r]; setEditAllowed();};
                ...
                window.removeRole = function (r) {session.AgentAccess.MemberRoles.remove(r); setEditAllowed();};
                window.removePermission = function (r) {session.Permissions.remove(r); setEditAllowed();};
                window.pmSession = session;
        }
        ...
        function addAdminFeatures() {
            ...
            function setRegionCode(code) {
                ...
                //we load 'helptext.js?v=' + window.PM_HELPTEXT_VERSION multiple times, so we need some extra randomness to stop caching
                var rnd = Math.floor(Math.random() * 100) + '';

                //the help javascript file must be EXECUTED by the browser, so we create a script tag and add it in.
                var tag=document.createElement('script');
                tag.setAttribute("type","text/javascript");
                tag.setAttribute("src", 'api/meta/' + (self.helpRegionCode ? (self.helpRegionCode + '/') : '') + 'helptext.js?v=' + window.PM_HELPTEXT_VERSION + rnd);
                // console.log(tag);
